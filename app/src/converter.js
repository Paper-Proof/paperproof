const drawNewHypotheses = (hypsBefore, hypsAfter) => {
  ...
  const hypsBeforeThatDisappeared = hypsBefore.filter(
    (hyp) => !hypsAfterUsernames.includes(hyp.username)
  );
  const hypsAfterThatAppeared = hypsAfter.filter(
    (hyp) => !hypsBeforeUsernames.includes(hyp.username)
  );
  ...
  hypsAfter.forEach((hypAfter) => {
    const hypBeforeWithSameUsername = hypsBefore.find(
      (hypBefore) => hypBefore.username == hypAfter.username
    );
    if (
...
const getWindowByGoalId = (windows, goalId) => {
  return windows.find((w) => w.goalNodes.find((g) => g.id === goalId));
};
...
export const toEdges = (infoTreeVast) => {
  ...
  infoTree.forEach((tactic) => {
    ...
    const irrelevantGoalNames = tactic.goalsBefore
      .filter((goalBefore) => goalBefore.username !== mainGoalBefore.username)
      .map((goalBefore) => goalBefore.username);
    const relevantGoalsAfter = tactic.goalsAfter.filter(
      (goalAfter) => !irrelevantGoalNames.includes(goalAfter.username)
    );
    ...
    if (relevantGoalsAfter.length === 0) {
      ...
      relevantGoalsAfter.forEach((goalAfter, index) => {
        ...
        let [prettyHypNodes, prettyHypArrows] = drawNewHypotheses(
          hypsBefore,
          hypsAfter
        );
