import { HTMLContainer, SVGContainer, TLComponentProps, TLShapeUtil } from '@tldraw/core'
import { CenterHandle } from '@tldraw/core/src/components/Bounds/CenterHandle'
...
function createRect(p: number[], sz: number[], text: string, type: 'exists' | 'forall'): Rect {
  return {
    center: p.slice(),
    size: sz.slice(),
    text,
...
function genRectsForSize(size: number[], text: string[], name?: string): Rect[] {
  const pad = [5, 5]
  let p = [0, 0]
  let sz = [80, 25]
  const rects: Rect[] = [
    createRect(p, sz, text.length == 1 ? `${name}: ${text[0]}` : text[0], 'exists'),
  ]
...
export function getRects(text: string[], name?: string): [number[], Rect[]] {
  const size: Vec2 =
    text.length == 1 ? [120, 100] : text[text.length - 1].includes('∀') ? [175, 100] : [300, 100]
...
function buildProofLayout(tree: ProofTree, intros: string[] = []): ProofLayout {
  if (tree.action?.includes('intro')) {
    const intros = tree.children.flatMap((c) => ('type' in c ? [`(${c.name}: ${c.type})`] : []))
  ...
  const size = sizes.reduce(([x0, y0], [x, y]) => [x0 + x + 10, Math.max(y0, y)], [10, 0])
  return {
    expanded: true,
    size: [Math.max(100, size[0]), Math.max(50, size[1] + 50 + (intros.length > 0 ? 30 : 0))],
        ...
        .map((shape, index) => {
          const [fontSize, textY] = (() => {
            ...
            if (fontSize1 < fontSize2) {
              ...
              return [fontSize2, shape.size[1] * (shape.kf < 0.5 ? 0.3 : shape.kf)]
            }
          })()
          const center = [pos[0] + shape.center[0] + 5, pos[1] + shape.center[1] - size[1] + 5]
          return (
            <g key={index}>
              <rect
                x={center[0]}
    ...
    return (
      <>
        <g {...events} onClick={() => setExpanded(!expanded)}>
          ...
          {layout.intros.length > 0 || layout.defns.length > 0 ? (
            <text x={pos[0] + 10} y={pos[1] - layout.size[1] + 30}>
              {layout.intros.length > 0 ? `${forallSymbol} ${layout.intros.join(', ')}` : ''}
        ...
        {(expanded ? layout.children : []).map((child, index) => (
          <FrameComponent layout={child} pos={childrenPos[index]} events={events}></FrameComponent>
        ))}
      </>
    )
  } else {
    return (
      <g {...events} onClick={() => setExpanded(!expanded)}>
        <TypeComponent
          ...
          vars={[...layout.intros.map((s) => `${forallSymbol} ${s}`), layout.endType]}
        ></TypeComponent>
      </g>
    )
    /*   return (
    ...
    if (shape.proofTree) {
      ...
      return (
        <SVGContainer ref={ref} {...events} pointerEvents="all">
          <FrameComponent layout={layout} pos={[0, 500]} events={events}></FrameComponent>
        </SVGContainer>
      )
    }
